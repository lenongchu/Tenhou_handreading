...
| | `fls` | 索子副露 | 副露均为 s |
| | `flm` | 万字副露 | 副露均为 m |
| | `lmfl` | 两面副露 | 中张连续搭子吃（23、34、45、56、67、78），可形成两种吃的形式；12/89/嵌搭/边搭仅一种形式，不匹配 |

- **用法 1（副露区域约束）**：在「副露区域」输入 `fl`、`flp 4mc3m5m` 等，目标玩家必须满足所有约束（AND）。
- **用法 2（舍牌模式）**：如 `fl-3m` 表示「任意副露后打出 3m」；`lmfl-$` 表示「两面副露后手切任意牌」。
...
**前段有打匹配逻辑**：在主模式匹配点之前的舍牌序列中，检查是否存在**任意长度为 L 的连续子序列**包含该模式（L 为前段有打模式的 token 数）。单元素模式（如 `[37]mOR[37]s`）也需检查所有连续子序列，而非仅后缀。实现见 `equivalent_variants.match_discard_pattern_contained`。
...
- **关联牌定义**：打出的**数牌**与当时手牌中该花色的**搭子**（两面/边张/嵌张）或**对子**在数字上距离 ≤ 2 时，视为关联牌。即手牌中存在与打出牌数字差 ≤ 2 的搭子或对子，则该舍牌计为「关联」。
...
- **样本生成**：单巡目范围可生成样本；矩阵分析（M>1 巡目）时不生成样本。
