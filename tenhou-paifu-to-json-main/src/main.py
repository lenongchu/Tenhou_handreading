...
# owari 在最后一个 AGARI/RYUUKYOKU 中；若其后有 BYE，nowRound[-1] 为 BYE 无 owari，需往前找
owariRaw = None
for evt in reversed(nowRound):
    if evt[0] in ("AGARI", "RYUUKYOKU") and "owari" in evt[1]:
        owariRaw = evt[1]["owari"]
        break
if owariRaw is None or len(owariRaw.split(",")) < 7:
    raise KeyError("no owari in any AGARI/RYUUKYOKU")
owariData = list(owariRaw.split(","))
# owari 格式可能含小数如 "85,-41.0,311,11.0"，取偶数位为分数（单位为 100 点）
owari = listmap(lambda x: int(float(x)), [owariData[0], owariData[2], owariData[4], owariData[6]])
ans = {
    "type"     : type,
    "lobby"    : lobby,
    "id"       : id,
